...
(function(){
  ...
  async function saveWork(type,title,payload){
    const mode=document.getElementById("modernMode")?.value||"free",classId=document.getElementById("modernClass")?.value||null,
      ...
      createdAt=payload.createdAt||now,finalClassId=mode==="class"?classId:null,finalPayload={...payload,id:undefined};
    await enregistrerLigne("eps_saved_tool_works",{
      ...
      class_id:finalClassId,period_number:period,created_at:createdAt,updated_at:now,deleted:false
    });
    return {id,type,title:title||"Travail",classId:finalClassId,period,payload:finalPayload,createdAt,updatedAt:now};
  }
