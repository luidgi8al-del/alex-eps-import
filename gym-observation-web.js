...
(function(){
  ...
  function scoreFigure(f){if(!f||f.difficulte==null)return 0;if(f.erreur==="chute")return 0;return Math.max(0,(DIFFICULTES[f.difficulte]||0)-(ERREURS[f.erreur]||0))}
  function scoreTotal(figures){return (figures||[]).reduce((s,f)=>s+scoreFigure(f),0)}
  function encode(figures){return "gym:"+JSON.stringify({version:1,figures})}
  function decode(raw,n){try{const parsed=JSON.parse(String(raw||"").replace(/^gym:/,""));const figures=Array.isArray(parsed.figures)?parsed.figures:[];return Array.from({length:n},(_,i)=>figures[i]||figureVide())}catch{return Array.from({length:n},()=>figureVide())}}
  const state={classId:"",period:1,numFigures:4,values:{},sessionId:null,resultIds:{},createdAt:null,saved:[],busy:false};
  ...
  function resetForClass(){state.values={};state.sessionId=null;state.resultIds={};state.createdAt=null}
  ...
  function drawSetup(){toolPanel.innerHTML=`<div class="fitness-web">${hero("Difficulté par figure, erreurs par élève")}${contextHtml()}<section class="field-tool-card"><div class="fitness-auto-size"><button type="button" id="gymObsMoins" aria-label="Moins de figures">−</button><span>Évaluer <b id="gymObsNombre">${state.numFigures}</b> figure${state.numFigures>1?"s":""}</span><button type="button" id="gymObsPlus" aria-label="Plus de figures">+</button></div></section><button id="gymObsStart">Ouvrir l’observation</button>${savedHtml()}<p class="muted">Les observations enregistrées sont automatiquement rangées dans Tests EPS de la classe concernée.</p></div>`;
    const mode=document.getElementById("gymObsMode"),cls=document.getElementById("gymObsClass");cls.onchange=async()=>{state.classId=cls.value;mode.value=cls.value?"class":"free";resetForClass();await loadToolStudents(state.classId);ensureValues();drawSetup()};document.getElementById("gymObsPeriod").onchange=e=>state.period=+e.target.value;
  ...
  function celluleHtml(sId,i){const f=(state.values[sId]||[])[i]||figureVide(),score=scoreFigure(f);return `<button type="button" class="gym-obs-cell" data-gym-cell="${sId}|${i}">${f.difficulte?`<b>${f.difficulte}</b><small>${f.erreur!=="aucune"?ERREUR_LABEL[f.erreur].split(" ")[0]:""}</small><em>${fr(score,1)}</em>`:`<span class="muted">—</span>`}</button>`}
  ...
  async function openSaved(id,duplicate=false){const saved=state.saved.find(s=>String(s.id)===String(id));if(!saved)throw new Error("Cette observation est introuvable sur cet appareil : rechargez la page, elle n’a peut-être pas encore été reçue.");state.classId=saved.class_id;state.period=+saved.period_number||1;state.sessionId=duplicate?null:saved.id;state.createdAt=duplicate?null:saved.created_at;state.resultIds={};state.values={};await loadToolStudents(state.classId);let rows=await lireTable("eps_test_results",`eps_test_results?session_id=eq.${saved.id}&deleted=eq.false&select=*`,{ou:r=>String(r.session_id)===String(saved.id)&&!r.deleted});
    ...
    rows.forEach(r=>{state.values[r.student_id]=decode(r.input_unit,state.numFigures);if(!duplicate)state.resultIds[r.student_id]=r.id});
  ...
  const exportRows=()=>toolStudents.map(s=>{const figures=state.values[s.id]||[];return [s.last_name||"",s.first_name||"",...figures.map(f=>f.difficulte?`${f.difficulte}${f.erreur!=="aucune"?" · "+ERREUR_LABEL[f.erreur].split(" ")[0]:""} (${fr(scoreFigure(f),1)})`:"—"),fr(scoreTotal(figures),1)]});
