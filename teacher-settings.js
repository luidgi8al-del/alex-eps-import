...
(function () {
  ...
  async function openSettings() {
    ...
    document.getElementById("settingsBody").innerHTML=
      settingsSection("offlineSection","Utiliser sans connexion",`<p>Préparez cet appareil avant de partir au gymnase ou sur le terrain.</p><button id="prepareOfflineBtn">Préparer cet appareil pour le mode hors connexion</button><p id="offlinePreparationStatus" role="status" aria-live="polite">Vérification…</p><p class="muted">Classes, élèves, planning, évaluations et données AS synchronisées sont conservés sur cet appareil. Les e-mails, l’administration et les ressources externes nécessitent Internet. Certains écrans complémentaires restent en ligne. Ne vous déconnectez pas avant une utilisation sans réseau : la déconnexion efface les copies locales.</p>`)+
    ...
    const offlineStatus = document.getElementById("offlinePreparationStatus");
    offlinePreparationStatus().then(text=>{offlineStatus.textContent=text;}).catch(e=>{offlineStatus.textContent=e.message;});
    bind("prepareOfflineBtn",()=>prepareOfflineDevice(text=>{offlineStatus.textContent=text;}).catch(e=>{offlineStatus.textContent=e.message;}));
