...
async function drawStopCourseTest(test) {
  ...
  // Tableau du socle : la colonne des noms reste en vue pendant qu'on fait defiler les colonnes,
  // comme dans Condition physique. Le "-1" repare un manque : on ne pouvait qu'ajouter un arret,
  // jamais corriger celui compte en trop.
  const colonnes=[...(stopGroupCount>1?[{titre:"Groupe"}]:[]),{titre:"Arrêts",aide:"Toucher pour +1"},{titre:"Corriger"}];
  const lignes=visible.map(s=>{const n=stopCourseCounts[s.id]||0;return {eleve:s,
    sousTitre:stopGroupCount>1?`Groupe ${groupOf(s)}`:"",
    cellules:[
      ...(stopGroupCount>1?[`<select data-stop-student-group="${s.id}">${Array.from({length:stopGroupCount},(_,i)=>`<option value="${i+1}" ${groupOf(s)===i+1?'selected':''}>${i+1}</option>`).join('')}</select>`]:[]),
      `<button class="stop-student" data-stop-student="${s.id}">${n} <i>+1</i></button>`,
      `<button class="secondary" data-stop-moins="${s.id}" ${n?'':'disabled'} style="width:auto;margin:0">−1</button>`
    ],
    total:stopBadge(n)}});
  host.innerHTML=`<details class="test-protocol" open><summary>Protocole</summary><p>${test.protocol}</p></details>${stopModeSelectorHtml()}<div class="running-group-tabs"><button class="${stopActiveGroup===0?'':'secondary'}" data-stop-group="0">Tous</button>${Array.from({length:stopGroupCount},(_,i)=>`<button class="${stopActiveGroup===i+1?'':'secondary'}" data-stop-group="${i+1}">Groupe ${i+1}</button>`).join('')}<label>Groupes<select id="stopGroupCount">${[1,2,3,4,5,6,7,8].map(n=>`<option value="${n}" ${n===stopGroupCount?'selected':''}>${n===1?'Sans groupe':n+' groupes'}</option>`).join('')}</select></label></div><div class="fitness-web">${socleTableauHtml(colonnes,lignes,stopMode==="couleur"?"Couleur":"Note /20")}</div><div class="running-save-actions"><button id="stopSave">${stopSessionId?'Enregistrer les modifications':'Enregistrer dans Tests EPS'}</button><button class="secondary" id="stopSaveAs">Enregistrer comme nouveau test</button><button class="secondary" id="stopReset">↺ Réinitialiser</button></div><section class="field-tool-card"><h3>Tests enregistrés</h3><div class="running-session-list">${stopSessions.map(s=>`<div class="running-session-card"><button data-stop-session="${s.id}"><b>${test.label}</b><small>${new Date(s.created_at).toLocaleString('fr-FR')} · modifiable</small></button><button class="danger" data-stop-delete="${s.id}">Supprimer</button></div>`).join('')||'<span class="muted">Aucun test enregistré.</span>'}</div></section>`;
  document.getElementById("stopMode").onchange=e=>{stopMode=e.target.value;drawStopCourseTest(test)};
  document.getElementById("stopSeuil").onchange=e=>{stopSeuil=Math.max(1,+e.target.value||1);drawStopCourseTest(test)};
  ...
  host.querySelectorAll('[data-stop-moins]').forEach(b=>b.onclick=()=>{const id=b.dataset.stopMoins;stopCourseCounts[id]=Math.max(0,(stopCourseCounts[id]||0)-1);drawStopCourseTest(test)});
  // Repartir de zero sans quitter l'outil : les comptes s'effacent, le test deja enregistre
  // reste en place tant qu'on ne l'ecrase pas.
  document.getElementById('stopReset').onclick=()=>{if(!confirm('Remettre tous les compteurs à zéro ?'))return;Object.keys(stopCourseCounts).forEach(k=>delete stopCourseCounts[k]);drawStopCourseTest(test)};
