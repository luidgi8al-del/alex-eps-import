...
function resetRunningSession(){runningSessionId=null;runningCreatedAt=null;runningSessionRecord=null;runningResultRecords={};runningExpandedStudent=null;Object.keys(runningValues).forEach(k=>delete runningValues[k]);toolStudents.forEach(s=>runningValues[s.id]=Array(runningCount).fill(''))}
...
async function openRunningSession(id,test){const saved=runningSessions.find(s=>String(s.id)===String(id));if(!saved)return;const rows=await lireTable('eps_test_results',`eps_test_results?session_id=eq.${id}&deleted=eq.false&select=*`,{ou:r=>String(r.session_id)===String(id)&&!r.deleted});runningSessionId=saved.id;runningCreatedAt=saved.created_at;runningSessionRecord=saved;runningResultRecords={};Object.keys(runningValues).forEach(k=>delete runningValues[k]);const first=rows[0]?.input_unit||'',parts=first.split(':');if(parts[0]==='runs'){runningDistance=+parts[1]||500;runningCount=Math.max(1,Math.min(10,+parts[2]||3))}toolStudents.forEach(s=>runningValues[s.id]=Array(runningCount).fill(''));rows.forEach(r=>{runningResultRecords[r.student_id]=r;const times=String(r.input_unit||'').split(':times:')[1];if(times!=null)runningValues[r.student_id]=times.split('|').slice(0,runningCount).concat(Array(runningCount).fill('')).slice(0,runningCount)});paintRunningSeriesTest(test)}
...
async function saveRunningSeries(test,asNew){const cls=toolClasses.find(c=>c.id===toolClassId),id=asNew||!runningSessionId?crypto.randomUUID():runningSessionId,now=new Date().toISOString(),created=asNew||!runningCreatedAt?Date.now():runningCreatedAt;const selected=toolStudents.map(s=>[s,runningSummary(s),runningValues[s.id]]).filter(([,r,raw])=>r||raw.some(v=>v===RUNNING_REMOVED));const sessionRow=asNew||!runningSessionRecord?{id,user_id:session.user_id,class_id:toolClassId,period_number:epsTestPeriod,test_name:test.label,created_at:created,class_label:cls?.name||'',updated_at:now,deleted:false}:{...runningSessionRecord,updated_at:now,deleted:false};await enregistrerLigne('eps_test_sessions',sessionRow);const kept=new Set();for(const [s,r,raw] of selected){const old=!asNew?runningResultRecords[s.id]:null,resultId=old?.id||crypto.randomUUID();kept.add(String(resultId));const valid=r?.valid||[],active=raw.filter(v=>v!==RUNNING_REMOVED).length;await enregistrerLigne('eps_test_results',{...(old||{}),id:resultId,user_id:old?.user_id||session.user_id,session_id:id,student_id:s.id,input_value:r?.avg||0,result_value:r?.total||0,input_unit:`runs:${runningDistance}:${runningCount}:group:-1:times:${raw.join('|')}`,result_unit:`${valid.length>1?'/12':'/6'}${valid.length<active?' · en cours':''}`,updated_at:now,deleted:false})}if(!asNew)for(const old of Object.values(runningResultRecords))if(!kept.has(String(old.id)))await enregistrerLigne('eps_test_results',{...old,deleted:true,updated_at:now});runningSessionId=id;runningCreatedAt=created;runningSessionRecord=sessionRow;await loadRunningSessions();await openRunningSession(id,test);const msg=document.getElementById('runningMsg');if(msg)msg.textContent=asNew?'Nouvelle session enregistrée.':'Session mise à jour.'}
